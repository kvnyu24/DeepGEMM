...
class CoreTests(unittest.TestCase):
    ...
    def construct(self, m: int, k: int, n: int) -> \
    ...
    def construct_grouped(self, num_groups: int, m: int, k: int, n: int, is_masked: bool) -> \
            Tuple[Tuple[torch.Tensor, torch.Tensor], Tuple[torch.Tensor, torch.Tensor], torch.Tensor, torch.Tensor]:
        ...
        for i in range(num_groups):
            x_fp8[0][i], x_fp8[1][i] = self.per_token_cast_to_fp8(x[i])
            y_fp8[0][i], y_fp8[1][i] = self.per_block_cast_to_fp8(y[i])

        # For non-masked input, we must merge the group and M dims
        if not is_masked:
            x_fp8 = (x_fp8[0].view(-1, k), self.per_token_cast_to_fp8(x.view(-1, k))[1])
    ...
    def test_m_grouped_gemm_contiguous(self) -> None:
        ...
        for num_groups, m, k, n in ((4, 8192, 7168, 4096), (4, 8192, 2048, 7168), (8, 4096, 7168, 4096), (8, 4096, 2048, 7168)):
            ...
            x_fp8, y_fp8, out, ref_out = self.construct_grouped(num_groups, m, k, n, is_masked=False)
    ...
    def test_m_grouped_gemm_masked(self) -> None:
        ...
        for num_groups, m in ((1, 1024), (2, 512), (4, 256)):
            for k, n in ((7168, 4096), (2048, 7168), ):
                ...
                for i in range(10):
                    x_fp8, y_fp8, out, ref_out = self.construct_grouped(num_groups, m, k, n, is_masked=True)
